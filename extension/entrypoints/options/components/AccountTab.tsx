    ...
    <div className="space-y-6">
      <section className="bg-card rounded-xl border border-border p-6">
        <div className="flex items-center justify-between mb-4">
          ...
          <button onClick={onSignOut} className="flex items-center gap-2 text-sm text-foreground-muted hover:text-red-400">
      ...
      <section className="bg-card rounded-xl border border-border p-6">
        <h2 className="text-lg font-medium text-foreground mb-4">Cloud Sync</h2>
        <div className="flex items-center justify-between p-4 bg-card-elevated rounded-lg">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-sm font-medium text-foreground">Sync Data to Cloud</p>
              <p className="text-xs text-foreground-muted">{leadsCount} leads stored locally</p>
            </div>
          </div>
          <button onClick={onSyncToCloud} className="px-4 py-2 bg-foreground text-background text-sm font-medium rounded-lg hover:bg-accent-hover">
            Sync Now
          </button>
        </div>
      </section>
      ...
      {subscription?.plan === "free" && (
        <section className="bg-card-elevated rounded-xl p-6 border border-border">
          ...
          <button className="px-4 py-2 bg-foreground text-background font-medium rounded-lg hover:bg-accent-hover">View Plans</button>
        </section>
      )}
    </div>
  );
...
function AuthForm({ authError, authLoading, onGoogleAuth }: AuthFormProps) {
  return (
    <section className="bg-card rounded-xl border border-border p-6">
      ...
      {authError && (
        <div className="flex items-center gap-2 p-3 bg-red-500/10 text-red-400 rounded-lg mb-4 text-sm">
...
function GoogleButton({ onClick, disabled }: { onClick: () => void; disabled: boolean }) {
  return (
    <button
      onClick={onClick}
      disabled={disabled}
      className="w-full py-2.5 border border-border bg-card text-foreground-secondary font-medium rounded-lg hover:bg-card-elevated hover:text-foreground disabled:opacity-50 flex items-center justify-center gap-2"
    >
